<x-app-layout>
    ...
    <div class="col-span-12 flex flex-col items-center gap-4" x-data="homeManager('{{ $user->username }}', {{ $isMe ? 'true' : 'false' }})">
        ...
        {{-- Canvas --}}
        <div
            class="home-canvas relative w-full max-w-[928px] h-[1360px] border border-gray-700 rounded-lg overflow-hidden bg-cover bg-center bg-no-repeat"
                            ...
                            <div class="flex w-full min-h-0">
                                <div class="w-44 shrink-0 border-r border-gray-700 p-2 flex flex-col gap-0.5">
                                    <template x-for="t in ['stickers','notes','widgets','backgrounds']" :key="t">
                                        <button class="text-left px-3 py-1.5 rounded text-sm capitalize transition" :class="invTab === t ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-700'" @click="invTab = t; invActive = null; placeQty = 1" x-text="t"></button>
                                    </template>
                                </div>
                                    ...
                                    <template x-if="!invLoading">
                                        <div class="flex flex-wrap gap-1.5">
                                            <template x-for="item in invItems()" :key="item.home_item_id">
                                                <div class="w-16 h-16 border rounded flex items-center justify-center cursor-pointer relative transition" :class="invActive?.home_item_id === item.home_item_id ? 'border-[#eeb425] bg-[#eeb425]/10' : 'border-gray-600 hover:border-gray-400'" @click="invActive = item; placeQty = 1" @dblclick="quickPlace(item)">
                                ...
                                <div class="w-44 shrink-0 border-l border-gray-700 p-3 flex flex-col">
                                    ...
                                    <template x-if="!invActive"><p class="text-gray-500 text-xs text-center mt-8">{{ __('Select an item to place it on your home.') }}</p></template>
                                </div>
                            </div>
                        </template>
                                            ...
                                            <template x-if="!shopLoading">
                                                <div class="flex flex-wrap gap-1.5">
                                                    <template x-for="item in shopItems" :key="item.id">
                                                        <div class="w-16 h-16 border rounded flex items-center justify-center cursor-pointer relative transition" :class="shopActive?.id === item.id ? 'border-[#eeb425] bg-[#eeb425]/10' : 'border-gray-600 hover:border-gray-400'" @click="pickShop(item)">
                                ...
                                <div class="w-44 shrink-0 border-l border-gray-700 p-3 flex flex-col">
                                    <template x-if="shopActive">
                                        <div class="flex flex-col items-center gap-2 text-center">
                                            ...
                                            <button class="w-full border-2 border-green-500 bg-green-600 hover:bg-green-700 text-white rounded font-semibold text-sm py-1.5 transition mt-1 disabled:opacity-50" @click="buyAndPlace()" :disabled="buying">
                                                {{ __('Buy & Place') }}
                                            </button>
                                        </div>
                                    </template>
                                    <template x-if="!shopActive"><p class="text-gray-500 text-xs text-center mt-8">{{ __('Select an item to see details.') }}</p></template>
                                </div>
                            </div>
                        </template>
