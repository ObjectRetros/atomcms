...
The draft implements full/headless route registration, shared operations and explicit public projections, versioned adapters across the feature map, JSON access restrictions, cookie/CORS authentication, Atom 2FA enrollment JSON, independent housekeeping assets, CLI setup/mode conversion, and the separate Vue Dusk frontend requested during implementation. The Vue example lives at `examples/dusk-vue`, with its own package, copied public artwork, generated API types, and no PHP/database dependency.

The checked-in OpenAPI schema is the external wire contract; native PHP DTOs remain internal application boundaries. `npm run api:generate` generates both the published definitions and the example's independent copy. `npm run api:check` compiles schemas, compares generated files, and checks Laravel route coverage. Representative Pest requests validate real response bodies with AJV rather than repeating field assertions. CI builds both PHP themes, independent housekeeping, and the Vue example.
